...
const AuthModal = ({ mode, onClose, onSwitchMode, onAuthSuccess }) => {
  ...
  const [currentAuthStep, setCurrentAuthStep] = useState(mode); 
  ...
  const validateForm = () => {
    ...
    // Password always required for login/signup
      ...
      if (currentAuthStep === 'login') {
        ...
        authResponse = await supabase.auth.signUp({
          ...
          options: {
            ...
            emailRedirectTo: import.meta.env.VITE_FRONTEND_URL + '/verified.html', // Redirect to a page that handles session
          }
        });
        ...
        if (user && !session) {
          // User created, but email verification is required (link sent)
          setInfoMessage('Account created! Please check your email for a verification link to confirm your account.');
  ...
  // NEW: Handle resending verification link
      ...
      <div className="modal-content">
        <div className="modal-header">
          <h2 className="modal-title">
            {currentAuthStep === 'login' ? 'Welcome Back' : 'Create Account'}
          </h2>
        ...
        <form className="auth-form" onSubmit={handleSubmit}>
          ...
          <button
            ...
            disabled={isLoading}
          >
            {isLoading ? 'Please wait...' : (currentAuthStep === 'login' ? 'Sign In' : 'Create Account')}
          </button>
        </form>

        <div className="auth-switch">
          <span>
            {currentAuthStep === 'login' ? "Don't have an account? " : "Already have an account? "}
          </span>
          <button
            type="button"
            ...
            disabled={isLoading}
          >
            {currentAuthStep === 'login' ? 'Sign Up' : 'Sign In'}
          </button>
        </div>
      </div>
    </div>
